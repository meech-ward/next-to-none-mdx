...
Every time our application interacts with the database, it's going to use [Drizzle](https://orm.drizzle.team/docs/overview). That means we're not really going to be writing SQL, instead we will write typescript.
...
<SideBySide ratio="1">
Here is what the equivalent SQL would look like:
...
This code is defining a table and a [Postgres enum](https://www.postgresql.org/docs/current/datatype-enum.html).
...
This code adds foreign key constraints to the other tables. It also has a self-referencing foreign key constraint for the `replyId` column which requires us to explicitly set the return type because of TypeScript limitations.
...
We have the schema defined for our three tables, that's great, and we could start writing out some queries in TypeScript using Drizzle. However, we really need to add these tables to our Neon database before we can actually add any data or run any queries. We can do this by running a migration or pushing the schema to the database.
...
To do this, we need to use another library called `drizzle-kit` 
...
We can configure Drizzle Kit using a `drizzle.config.ts` file in the root of our project. This file will be used to tell Drizzle Kit where to find our schema files, what database driver to use, and where to output the generated files.
...
All `drizzle-kit` commands we run will use this as the default configuration. It will read the schema files from the schema directory and output any generated files to the `drizzle` directory in the root of our project.

Notice that we're using the `MIGRATION_DATABASE_URL` from the [neon roles](/neon-roles) section.

<Instruction>
  <Instruction.Action step={11}>
    If you're not using bun, install `dotenv` as a dev dependancy.
  </Instruction.Action>  
  <Instruction.Implementation>
    <Tabs>
      <Tab name='npm'>
