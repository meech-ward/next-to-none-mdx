...
<Tabs>
  ...
  <Tab name="node">
   ...
   const password = crypto.randomBytes(16).toString('base64').replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '');
   console.log(password);
   ```
  </Tab>
  <Tab name="browser js">
    ```js
   function generateUrlSafePassword(length = 16) {
   const array = new Uint8Array(length);
   window.crypto.getRandomValues(array);å
   ...
   const password = generateUrlSafePassword();
   console.log(password);
   ```
  </Tab>
  <Tab name="python">
...
-- Grant permissions on the "neon" database
GRANT ALL PRIVILEGES ON DATABASE "neon" TO migration_role;
...
```
postgres://migration_role:your_password_here@<PGHOST>/neon
```
...
<Instruction.Implementation>
```sh
MIGRATION_DATABASE_URL=postgres://migration_role:your_password_here@<PGHOST>/neon
```
</Instruction.Implementation>
</Instruction>
...
```
postgres://app_role:your_password_here@<PGHOST>/neon
```
...
<Instruction.Implementation>
```sh
DATABASE_URL=postgres://app_role:your_password_here@<PGHOST>/neon
```
</Instruction.Implementation>
</Instruction>
...
```sh
#focus(1:1)
MIGRATION_DATABASE_URL=postgres://migration_role:your_password_here@<PGHOST>/neon?sslmode=verify-full
DATABASE_URL=postgres://app_role:your_password_here@<PGHOST>/neon
```
</Instruction.Implementation>
</Instruction>
