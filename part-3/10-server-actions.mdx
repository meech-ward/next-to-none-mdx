...
There's a lot of useless questions that come up when trying to implment some sort of backend endpoint. 
...
So wouldn't it be nice if we could simplify this entire process and only focus on the backend code? No unessary questions or decisions, just focus on the core logic that we need to write.
