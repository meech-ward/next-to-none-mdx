...
* Submitting the form should run some code on a server with the ability to connect to the database 
* Writing the code and manually testing it should be easy. Not too many questions to ask or decisions to make. 
* Maybe it could be done using a client side JS fetch request, or fall back to a standard form POST request if JS is disabled. That could be a nice feature. 
...
<Note>
Server Actions are really new and still considered experimental, so we have to opt into using them.
</Note>

<Instruction ratio="2:3">
